...
function($scope, $http, $rootScope, companyService, $window, $timeout, $document) {
	...
	$scope.showCommandModal = function() {
			...
			if ($rootScope.visible_command) {
				...
				$rootScope.commandText = "bash <(wget -qO- https://raw.githubusercontent.com/agentinfraguard/agentglobe/master/scripts/agentInstaller.sh --no-check-certificate) '"+serverName+"' "+ $scope.project.id+" "+"LicenseKey";
